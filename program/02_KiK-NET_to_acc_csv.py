...
def main():
    ...
    for file in os.listdir(_input_path):
        ...
        if ext == '.EW2' or ext == '.NS2' or ext == '.UD2':
            files.append(file)
            print(file,"このファイルを変換します")
    print("KiK-netの元データをcsv形式の二列データで出力します。")
    ...
    for filename in files:
        ...
        _dir=filename[-3:-1]#lines[12].split()[1] ## ここがK-NETと違う
        print(station_code)
        print(_dir)
        print(samplingFreq_temp)
        #output_path = "{0}{1}".format(output_folder_path,station_code+"."+dir[0]+dir[2]+".acc.csv")
        output_path = "{0}/{1}".format(output_folder_path,station_code+"."+_dir+".acc.csv")
        
        ScaleFactor_temp=lines[13].split()[2]
        ...
        # print(data_list[48])
        start_num=49 ## ここがK-NETと違う
        if data_list[start_num][1:].isdigit() == False:
            print("備考が含まれているから一単語ずらす")
            start_num=50
        data = pd.Series(data_list[start_num:], dtype='float') #data = pd.Series(data_list[49:], dtype='float')
